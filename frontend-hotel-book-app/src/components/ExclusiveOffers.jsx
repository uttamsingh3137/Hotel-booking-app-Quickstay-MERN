    ...
    <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 xl:px-32 pt-20 pb-30'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12  '>
            {exclusiveOffers.map((item)=>(
                <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 p-6 pt-12 md:pt-16 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{backgroundImage : `url(${item.image})`}}>

                    <p className='px-3 py-1 absolute top-4 left-4 text-xs bg-white text-gray-800 font-medium rounded-full'>{item.priceOff}% OFF</p>
                    <div>
                        <p className='flex items-center gap-2 font-medium font-playfair'>{item.title}</p>
